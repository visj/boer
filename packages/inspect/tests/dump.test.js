...
function readHeader(buf) {
    ...
    return {
        magic:          String.fromCharCode(buf[0], buf[1], buf[2], buf[3], buf[4]),
        fmtVersion:     v.getUint8(5),
...
describe('dump — header', () => {
    ...
    test('magic bytes are boer\\0', () => {
        console.log(hdr.magic);
        expect(hdr.magic).toBe('boer\0');
    });
...
describe('dump — regex table', () => {
    ...
    test('regex flags are preserved (u flag is default)', () => {
        ...
        /* boer always creates patterns with the 'u' (unicode) flag */
